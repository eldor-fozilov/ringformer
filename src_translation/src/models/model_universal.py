...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_dim, ffn_dim, num_head, bias, dropout, layernorm_eps):
        ...
        self.layer_norm_01 = nn.LayerNorm(self.hidden_dim, eps=self.layernorm_eps)
        self.layer_norm_02 = nn.LayerNorm(self.hidden_dim, eps=self.layernorm_eps)
    ...
    def forward(self, x, mask):
        ...
        output = self.layer_norm_01(x + output)

        x = output
        output = self.positionWiseFeedForward(output)
        output = self.dropout_layer(output)
        output = self.layer_norm_02(x + output)
...
class Encoder(nn.Module):
    def __init__(self, config, tokenizer):#, device
        ...
        self.lev_layer = LevelEncoding_en(self.max_len, self.hidden_dim, 1, self.enc_num_layers)#self.device, 
    ...
    def forward(self, x, mask=None):
        ...
        for encoder in self.encoders:
            for i in self.level_en:
                ...
                output = output+self.lev_layer(output, i)
                _, output = encoder(output, mask)
...
class DecoderLayer(nn.Module):
    def __init__(self, hidden_dim, ffn_dim, num_head, bias, dropout, layernorm_eps):
        ...
        self.layer_norm_01 = nn.LayerNorm(self.hidden_dim, eps=self.layernorm_eps)
        self.layer_norm_02 = nn.LayerNorm(self.hidden_dim, eps=self.layernorm_eps)
        self.layer_norm_03 = nn.LayerNorm(self.hidden_dim, eps=self.layernorm_eps)
    ...
    def forward(self, x, enc_output, dec_causal_mask, enc_dec_mask):
        ...
        output = self.positionWiseFeedForward(output)
        output = self.dropout_layer(output)
        output = self.layer_norm_03(x + output)
...
class Decoder(nn.Module):
    def __init__(self, config, tokenizer):#, device
        ...
        self.emb_layer = Embeddings(self.vocab_size, self.hidden_dim, self.pad_token_id)
        self.pos_layer = PositionalEncoding(self.max_len, self.hidden_dim, self.pos_encoding)#, self.device
        self.lev_layer = LevelEncoding_de(self.max_len, self.hidden_dim, 1, self.dec_num_layers)#self.device, 
    ...
    def forward(self, x, enc_output, dec_causal_mask=None, enc_dec_mask=None):
        ...
        for decoder in self.decoders:
            for i in self.level_de:
                ...
                output = output+self.lev_layer(output, i)
                *_, output = decoder(output, enc_output, dec_causal_mask, enc_dec_mask)#self.lev_layer(output, i), #dec_self_attn_wts, cross_attn_wts, output
