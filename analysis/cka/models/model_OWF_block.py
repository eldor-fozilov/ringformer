...
class Iden(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.hidden_dim = hidden_dim
        self.id = nn.Parameter(torch.eye(self.hidden_dim), requires_grad = False)
        self.proj = F.linear
    
    def forward(self, x):
        return self.proj(x, self.id)

# a single encoder layer
class EncoderLayer_prev(nn.Module):
    def __init__(self, hidden_dim, ffn_dim, num_head, bias, dropout, layernorm_eps):
        super(EncoderLayer_prev, self).__init__()
        self.hidden_dim = hidden_dim
        self.ffn_dim = ffn_dim
        self.num_head = num_head
        self.bias = bias
...
class EncoderLayer_F(nn.Module):
    ...
    def forward(self, attn_wts, output):
        #attn_wts, output = self.self_attention(query=x, key=x, value=x, mask=mask)
        #output = self.dropout_layer(output)
        #output = self.layer_norm(x + output)
...
class Encoder(nn.Module):
    def __init__(self, config, tokenizer):#, device
        ...
        self.encoders = nn.ModuleList([EncoderLayer_A(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps) for _ in range(self.enc_num_layers)])#1
        self.FFN = EncoderLayer_F(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps)
        #self.Idens = nn.ModuleList([Iden(self.hidden_dim) for _ in range(self.enc_num_layers)])
    ...
    def forward(self, x, mask=None):
        ...
        for i, encoder in enumerate(self.encoders):
            ...
            attn_wts, output = encoder(output, mask)#self.lev_layer(output, i)
            attn_wts, output = self.FFN(attn_wts, output)
...
# a single decoder layer
class DecoderLayer_A(nn.Module):
    def __init__(self, hidden_dim, ffn_dim, num_head, bias, dropout, layernorm_eps):
        super(DecoderLayer_A, self).__init__()
        self.hidden_dim = hidden_dim
        self.ffn_dim = ffn_dim
        self.num_head = num_head
        self.bias = bias
...
# a single decoder layer
class DecoderLayer_F(nn.Module):
    def __init__(self, hidden_dim, ffn_dim, num_head, bias, dropout, layernorm_eps):
        super(DecoderLayer_F, self).__init__()
        self.hidden_dim = hidden_dim
        self.ffn_dim = ffn_dim
        self.num_head = num_head
        self.bias = bias
    ...
    def forward(self, dec_self_attn_wts, cross_attn_wts, output):
        #dec_self_attn_wts, output = self.masked_self_attention(query=x, key=x, value=x, mask=dec_causal_mask)
        #output = self.dropout_layer(output)
        #output = self.layer_norm(x + output)
...
class SSM(nn.Module):
    ...
    def forward(self, x, S, i):
        ...
        #        #S[j, k] = torch.matmul(aa[i-1], S[j, k])#aa@S[j, k]
        #        ss[j, k] = a@ss[j, k]#.to(x.device)
...
class Decoder(nn.Module):
    def __init__(self, config, tokenizer):#, device
        ...
        self.decoders = nn.ModuleList([DecoderLayer_A(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps) for _ in range(self.dec_num_layers)])#1
        #self.Idens = nn.ModuleList([Iden(self.hidden_dim) for _ in range(self.dec_num_layers)])
        #self.FFN = DecoderLayer_F(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps)
    ...
    def forward(self, x, enc_output, dec_causal_mask=None, enc_dec_mask=None):
        ...
        for i, decoder in enumerate(self.decoders):
            ...
            dec_self_attn_wts, cross_attn_wts, output = decoder(output, enc_output, dec_causal_mask, enc_dec_mask)#self.lev_layer(output, i)
            #dec_self_attn_wts, cross_attn_wts, output = self.FFN(dec_self_attn_wts, cross_attn_wts, output)
            output = self.Idens[i-1](output)
