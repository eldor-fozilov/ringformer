...
class Encoder(nn.Module):
    def __init__(self, config, tokenizer):
        ...
        self.level_en = torch.arange(1, self.enc_num_layers+1, requires_grad=False)
        self.encoders = nn.ModuleList([EncoderLayer_A(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps) for _ in range(self.enc_num_layers)])
...
class Decoder(nn.Module):
    def __init__(self, config, tokenizer):
        ...
        self.decoders = nn.ModuleList([DecoderLayer_A(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps) for _ in range(self.dec_num_layers)])
