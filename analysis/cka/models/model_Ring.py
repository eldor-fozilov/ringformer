...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.emb_layer(self.pos.unsqueeze(0))[:, :x.size(1)]
        

# level encoding layer
class LevelEncoding_en(nn.Module):
    def __init__(self, max_len, hidden_dim, lev_encoding, enc_num_layers):
        ...
        if self.lev_encoding:
            ...
            self.lev = self.lev.repeat(1, self.max_len)
...
class Encoder(nn.Module):
    def __init__(self, config, tokenizer):
        ...
        self.encoders = nn.ModuleList([EncoderLayer(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps) for _ in range(1)])
    ...
    def forward(self, x, mask=None):
        ...
        for encoder in self.encoders:
            for i in self.level_en:
                ...
                _, output = encoder(output, lev_info, self.layer_norms_01[i-1], lev_info_F, self.layer_norms_02[i-1], mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, info_01, layer_norm_01, enc_output, info_02, layer_norm_02, lev_info_f, layer_norm_03, dec_causal_mask, enc_dec_mask):
        ...
        x = output
        cross_attn_wts, output = self.enc_dec_attention(query=x, Q=info_02[0], key=enc_output, K=info_02[1], value=enc_output, V=info_02[2], mask=enc_dec_mask)
        output = self.dropout_layer(output)
        output = layer_norm_02(x + output)
...
class Decoder(nn.Module):
    def __init__(self, config, tokenizer):
        ...
        self.level_de = torch.arange(1, self.dec_num_layers+1, requires_grad=False)
        self.decoders = nn.ModuleList([DecoderLayer(self.hidden_dim, self.ffn_dim, self.num_head, self.bias, self.dropout, self.layernorm_eps) for _ in range(1)])
    ...
    def forward(self, x, enc_output, dec_causal_mask=None, enc_dec_mask=None):
        ...
        for decoder in self.decoders:
            for i in self.level_de:
                ...
                _, _, output = decoder(output, lev_info_SA, self.layer_norms_01[i-1], enc_output, lev_info_CA, self.layer_norms_02[i-1], lev_info_F, self.layer_norms_03[i-1], dec_causal_mask, enc_dec_mask)
